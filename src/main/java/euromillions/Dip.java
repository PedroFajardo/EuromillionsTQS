...
public class Dip {
    ...
    public static final int REQUIRED_NUMBERS_COUNT_FOR_BET = 5;
    public static final int REQUIRED_STARS_COUNT_FOR_BET = 2;
    ...
    public static Dip generateRandomDip() {
        ...
        for (int i = 0; i < REQUIRED_NUMBERS_COUNT_FOR_BET; i++) {
            int candidate = generator.nextInt(49) + 1;
            if (!randomDip.getNumbersColl().contains(candidate)) {
                randomDip.getNumbersColl().add(candidate);
            }
        }
        for (int i = 0; i < REQUIRED_STARS_COUNT_FOR_BET; i++) {
